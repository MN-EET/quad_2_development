...
def fetch_mn_consumers_forecast(db: str, destdir: str):
    ...
    mn_consumers_forecast = db_con.execute(f'SELECT * FROM main_forecast.mart_mn_consumers__24').fetchdf()
    db_con.close()
    # Write dataframe to destdir
    mn_consumers_forecast.to_csv(destdir + "/mn_consumers__24.csv", index = False)